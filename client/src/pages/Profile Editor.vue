                ...
                <material-card color="mint"
                    ...
                    <v-form>
                        <v-container py-0>
                            <v-layout wrap>
                                <v-flex xs12 md4>
                                    <v-text-field :value="todos.location.name"
                                                  label="Work Location"
                                                  class="purple-input" 
                                                  disabled/>
                                </v-flex>
                                <v-flex xs12 md4>
                                    <v-text-field :value="my_email = todos.email"
                                ...
                                <v-flex xs12 md6>
                                    <v-text-field :value="lname = todos.last_name"
                                                  @input="lname = $event"
                                                  label="Last Name"
                                                  class="purple-input" />
                                </v-flex>
                                <v-flex xs12 md12>
                                    <v-text-field label="Phone Number" class="purple-input" :value="my_phone = todos.phone" @input="my_phone = $event" />
                                </v-flex>
                                <v-flex xs12 md12>
                                    <v-text-field label="Work Address" disabled class="purple-input" :value="my_address = todos.location.street_Address" @input="my_address = $event" />
                                </v-flex>
                                <v-flex xs12 md4>
                                    <v-text-field label="City" class="purple-input" :value="todos.location.city" disabled/>
                                </v-flex>
                                <v-flex xs12 md4>
                                    <v-text-field label="State" class="purple-input" :value="todos.location.state" disabled/>
                                </v-flex>
                                <v-flex xs12 md4>
                                    <v-text-field :value="todos.location.country"
                                                  label="Country"
                                                  class="purple-input"
                                                  disabled/>
                                </v-flex>
...
<script>
    ...
    export default {
        ...
        methods: {
            async updateStaff() {
                await graphql(UPDATE_STAFF, { my_id: this.myStaffID, firstname: this.fname, lastname: this.lname, myemail: this.my_email, myphone: this.my_phone });
                console.log("i am updating id: " + this.myStaffID + " for " + this.fname + " " + this.lname);
            },
        },
        computed: {
            ...mapGetters({
        ...
        async asyncData() {
            try {
                ...
                const { data } = await graphql(GET_TODOS, { myid: { id: responseUser.decoded.id } });
