                    ...
                    <v-form>
                        <v-container py-0>
                            <v-layout wrap>
                                ...
                                <v-flex xs12 md12>
                                    <v-text-field label="Mobile Phone" class="purple-input" :value="myMobilePhone = todos.mobile_phone" @input="myMobilePhone = $event" />
                                </v-flex>
                                <v-flex xs12 md12>
                                    <v-text-field label="Home Phone" class="purple-input" :value="myHomePhone = todos.home_phone" @input="myHomePhone = $event" />
                                </v-flex>
                                <v-flex xs12 md12>
                                    <v-text-field label="Work Phone" class="purple-input" :value="myWorkPhone = todos.work_phone" @input="myWorkPhone = $event" />
                                </v-flex>
                                <ul>
                                    <li>
                                        Your Children
                                        <ul>
                                            <li v-for="value in todos.child">{{value.first_name}} {{value.last_name}}</li>
                                        </ul>
                                    </li>
                                </ul>
        ...
        methods: {
            async updateStaff() {
                await graphql(UPDATE_PARENT, {
                    my_id: this.myParentID, firstname: this.fname, lastname: this.lname, myemail: this.my_email, mymobilephone: this.myMobilePhone,
                    myhomephone: this.myHomePhone, myworkphone: this.myWorkPhone,
                });
                console.log("i am updating id: " + this.myParentID + " for " + this.fname + " " + this.lname);
            },
        },
        computed: {
            ...mapGetters({
